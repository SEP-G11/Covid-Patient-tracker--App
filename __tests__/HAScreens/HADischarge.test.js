...
let tree
test('snapshot HospitalAdmin Discharge page', async () => {
    act(() => { tree = create(<HospitalAdminDischarge navigation={navigation} route={route} />) })
...
test('Successfully Discharge', async () => {
    global.alert = jest.fn();
    const id = tree.root.findByProps({ testID: 'id_test' }).props;
    act(() => id.onChangeText("12636353574430220"));
